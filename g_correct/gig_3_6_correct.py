...
class Society():
	def __init__(self):
		...
		self.is_childnode		=	[False 	for _ in range(self.no_of_people + 1)]
		self.is_parentnode		=	[False 	for _ in range(self.no_of_people + 1)]
	...
	def one_met(self, a, b):
		if self.met[a]:
			...
			if self.is_childnode[b]:
				p 							=	self.parent[b]
				self.tree[p].append(a)
				self.parent[a]				=	p
				self.is_childnode[a]		=	True
				self.is_parentnode[a]		=	False
